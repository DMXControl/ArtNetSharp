...
public class ObjectTypesTests
{
    ...
    {
        UID uid = new UID(123141);
        PortAddress portAddress = new PortAddress(0, 0, 0);
        byte bindIndex = 0;
        var e = new RDMUID_ReceivedBag(portAddress, bindIndex, uid);

        Assert.That(e.LastSeen.Date, Is.EqualTo(DateTime.UtcNow.Date));
        e.Seen(portAddress, bindIndex);
        Assert.Multiple(() =>
        {
            Assert.That(e.LastSeen.Date, Is.EqualTo(DateTime.UtcNow.Date));
    ...
    public async Task TestPortConfig()
    {
        ...
        PortAddress portAddress = new PortAddress(1, 2, 3);
        byte bindIndex = 1;
        var portConfig = new PortConfig(bindIndex, portAddress, true, false);
        portConfig.AddDiscoveredRdmUIDs(portAddress, bindIndex, new UID(0x12345678));
        Assert.That(portConfig.GetReceivedRDMUIDs(), Has.Length.EqualTo(1));
        await Task.Delay(500);
        portConfig.AddDiscoveredRdmUIDs(portAddress, bindIndex, new UID(0x12345678));
        Assert.That(portConfig.GetReceivedRDMUIDs(), Has.Length.EqualTo(1));
        await Task.Delay(500);
        portConfig.AddDiscoveredRdmUIDs(portAddress, bindIndex, new UID(0xff345678));
        ...
        portConfig.AddDiscoveredRdmUIDs(portAddress, bindIndex, []);
        Assert.That(portConfig.DiscoveredRDMUIDs, Has.Count.EqualTo(0));
