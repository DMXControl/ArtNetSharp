...
#pragma warning restore CS0659 // Typ überschreibt Object.Equals(object o), überschreibt jedoch nicht Object.GetHashCode()
{
                    ...
                    in UID? defaulRespUID = null) : base()
    {
        ...
        if (bindIndex == 0 && bindIp.HasValue)
            throw new ArgumentOutOfRangeException($"If the argument {nameof(bindIndex)} is unset then the {nameof(bindIp)} shouls also be unset.");

        if (bindIndex != 0 && !bindIp.HasValue)
            throw new ArgumentOutOfRangeException($"If the argument {nameof(bindIndex)} is set then the {nameof(bindIp)} should also be set.");
    ...
    protected sealed override void fillPacketCore(ref byte[] p)
    {
        ...
        if (BindIp.HasValue)
        {
            p[207] = BindIp.Value.B1; // Bind IP 1
            p[208] = BindIp.Value.B2; // Bind IP 2
            p[209] = BindIp.Value.B3; // Bind IP 3
            p[210] = BindIp.Value.B4; // Bind IP 4
        }
    ...
    public override bool Equals(object obj)
    {
        return base.Equals(obj)
            ...
            && object.Equals(BindIp, other.BindIp)
