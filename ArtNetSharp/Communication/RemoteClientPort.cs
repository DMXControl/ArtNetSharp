...
public sealed class RemoteClientPort : INotifyPropertyChanged
{
    ...
    private void addControllerRdmUID(PortAddress portAddress, byte bindIndex, UID rdmuid)
    {
    ...
    internal void AddResponderRdmUIDs(PortAddress portAddress, byte bindIndex, params UID[] rdmuids)
    {
        if (rdmuids.Length == 0)
            return;
    ...
    internal void ProcessArtRDM(ArtRDM artRDM)
    {
        if (!artRDM.RDMMessage.Command.HasFlag(ERDM_Command.RESPONSE))
        {
            if (artRDM.Source.IsValidDeviceUID)
                addControllerRdmUID(artRDM.PortAddress, 0, artRDM.Source);
